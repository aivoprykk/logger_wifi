...
void print_local_time() {
	struct tm tm;
	get_local_time(&tm);
	WLOG(TAG, "NTP Time set: %d-%02d-%02d %02d:%02d:%02d", (tm.tm_year) + 1900,
...
void sntp_sync_time(struct timeval *tv) {
	...
	int ret = c_set_time_ts(tv->tv_sec, tv->tv_usec,
							(int8_t)g_rtc_config.gps.timezone);
#else
	...
	if (ret) {
		...
		WLOG(TAG, "NTP raw time: %d-%02d-%02d %02d:%02d:%02d, tvsec:%lld",
			 (my_time.tm_year) + 1900, (my_time.tm_mon) + 1, my_time.tm_mday,
			 my_time.tm_hour, my_time.tm_min, my_time.tm_sec, tv->tv_sec);
#endif
	}
	...
	// Time sync event removed - polling system handles display updates
	// regularly
	print_local_time();
}
// #endif
...
void initialize_sntp(float offset) {
	...
	if (wifi_context.s_wifi_event_group) {
		xEventGroupClearBits(wifi_context.s_wifi_event_group,
							 WIFI_TIME_SYNC_BIT);
	}
...
int wifi_wait_for_time_sync(uint32_t timeout_ms) {
	...
	EventBits_t bits =
		xEventGroupWaitBits(wifi_context.s_wifi_event_group, WIFI_TIME_SYNC_BIT,
